...
def get_descriptors_state( sharc_dat, rng, train_frac,n_molecules ):
    ...
    dict_properties.update({'Coulomb': False})
    ...
    descriptors = []
    all_dmats = transform_coordinates( all_atypes, all_geoms, all_molecules, natoms )
    ...
    for i in range(all_molecules):
      descriptor_1molec=([j for j in all_dmats[i][np.triu_indices(natoms,0)]])
      state2=np.zeros((len(descriptor_1molec),1))
      state2[:][:]=2.0
      state2=state2.flatten()
      descriptor_1molec2=np.multiply(descriptor_1molec,state2)
      state3=np.zeros((len(descriptor_1molec),1))
      state3[:][:]=3.0
      state3=state3.flatten()
      descriptor_1molec3=np.multiply(descriptor_1molec,state3)
      descriptor_3states=np.concatenate((descriptor_1molec,descriptor_1molec2,descriptor_1molec3))
      descriptors.append(descriptor_3states)
...
def get_descriptors( sharc_dat, rng, train_frac,n_molecules ):
    """get descriptor matrices by processing data from SHARC-xyz-output, return list of elements in every upper triangular descriptor matrix
    PARAMETERS:
    sharc_xyz: "output.xyz" produced by SHARC
    train_frac: fraction of training set, e.g. 0.9
    ...
    # transform every upper triangular descriptor matrix to list, append to list of descriptors for every state
    for i in range(all_molecules):
      descriptors.append([j for j in all_dmats[i][np.triu_indices(natoms,1)]])
    descriptors = np.array(descriptors)
...
def read_output_dat(sharc_dat):
    ...
    for step,read_dict in y:
      ...
      #soc as upper triangular matrix
      i=0
      Hamiltonian_full = np.zeros((nmstates,nmstates*2))
      for h1 in range(nmstates):
        for h2 in range(nmstates):
          Hamiltonian_full[h1][h2*2] = read_dict['Hamiltonian_real'][h1][h2]
          Hamiltonian_full[h1][h2*2+1] = read_dict['Hamiltonian_imag'][h1][h2]
      for soc_step_line in range(nmstates):
        for soc_step in range(soc_step_line*2+2,nmstates*2):
          i+=1
          soc_step_real_imag.append(Hamiltonian_full[soc_step_line][soc_step])
      all_soc_real_imag.append(soc_step_real_imag)
    ...
    all_soc = np.array(all_soc_real_imag)
    all_grad = np.array(all_grad)
    all_grad = all_grad/0.529177211
...
class output_dat:
  ...
  def __next__(self):
    ...
    Nac=[ [0 for i in range(3) ] for j in range(self.natoms*self.nmstates*self.nmstates) ]
     #nac_scaling i and j give the index used for getting Energy Ei and Ej for scaling the NAC vectors
    nac_scaling_i=-1
    nac_scaling_j=0
    for iline in range(self.nmstates*self.nmstates):
        nac_scaling_i+=1
        if nac_scaling_i >= int(self.nmstates):
          nac_scaling_i=0
          nac_scaling_j+=1
        E_forscaling_i=float(H_real[nac_scaling_i][nac_scaling_i])
        E_forscaling_j=float(H_real[nac_scaling_j][nac_scaling_j])
        delta_E=np.abs(E_forscaling_i-E_forscaling_j)
        if int(nac_scaling_j)==int(nac_scaling_j):
          delta_E=int(1)
    if self.nacindex == 1:
      for iline in range(self.nmstates*self.nmstates):
